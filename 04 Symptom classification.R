...
data <- master_data[-which(is.na(master_data$Improved_Symptoms)),]

# Remove columns that are not needed for this classification task
data[,c(60:71,74:76,78)] <- NULL

# Switch oucome factor levels
data$Improved_Symptoms <- factor(data$Improved_Symptoms,
                                 levels = c("yes", "no"))
...
table(classifier_train$Improved_Symptoms)

# Significant class imbalance here - address with SMOTE
set.seed(111)
balanced_training <- smote(Improved_Symptoms ~ .,
                           classifier_train,
                           perc.over = 2.5,
                           perc.under = 1.5)

table(balanced_training$Improved_Symptoms)
...
recipe <- recipe(Improved_Symptoms ~., data = balanced_training) %>% # Specify formula
          step_dummy(Gender, HandDominance, VibratingToolsYN,
           ...
           prep(training_data = balanced_training)
...
penalty_mixture_grid <- grid_regular(penalty(), # Penalty and mixture will both be tuned
                                     mixture(),
                                     levels = 10)
...
performanceResults <- function(workflow, training_data, test_data){
  ...
  p <-  fitted_workflow %>%
    ...
    bind_cols(test_data %>% select(Improved_Symptoms)) 
  ...
  stats <- function(data, indicies) {
    ...
    res <- c(
      preds %>%
        accuracy(as.factor(Improved_Symptoms), .pred_class) %>%
        select(.estimate) %>% 
        as.numeric(),
      
      preds %>%
        sensitivity(as.factor(Improved_Symptoms), .pred_class) %>%
        select(.estimate) %>% 
        as.numeric(),
      
      preds %>%
        specificity(as.factor(Improved_Symptoms), .pred_class) %>%
        select(.estimate) %>% 
        as.numeric()
    )
  }
  ...
  auc_boots <- function(auc_data, auc_indicies) {
    ...
    auc_preds %>%
      roc_auc(as.factor(Improved_Symptoms), .pred_yes) %>%
      select(.estimate) %>% 
      as.numeric()
  }
  ...
  p$Improved_Symptoms <- as.factor(p$Improved_Symptoms)
  
  conf_matrix <- p %>%
    conf_mat(truth = Improved_Symptoms, .pred_class)
  
  # and record ROC curve data.
  ROC_data <- auc_p %>%
    roc_curve(Improved_Symptoms, .pred_yes)
...
lr_results <- performanceResults(log_wfl, balanced_training, classifier_test)
...
# Results
xgb_results <- performanceResults(xgb_wf, balanced_training, classifier_test)
...
set.seed(111)
xg_mod_fit <- xgb_wf %>%
  fit(balanced_training) %>%
  pull_workflow_fit() # Define fitted model

X <- prep(recipe, classifier_test) %>%
  juice() %>%
  select(-Improved_Symptoms) %>%
  as.matrix() # Create a matrix of predictors to evaluate

shap_long_symptoms <- shap.prep(xgb_model = xg_mod_fit$fit, 
                                X_train = X)
                                 # Get SHAP values

shap.plot.summary(shap_long_symptoms) # Plot SHAP values
...
# Results
svm_results <- performanceResults(svm_wfl, balanced_training, classifier_test)
...
# Results
nnet_results <- performanceResults(nnet_wfl, balanced_training, classifier_test)
...
# Results
knn_results <- performanceResults(knn_wfl, balanced_training, classifier_test)
...
## CHAID with DASH 9, DASH 11 and EQ5D VAS, based on full dataset

# Create CHAID dataset

CHAID_dataset <- data %>% select(Improved_Symptoms, DASH11, DASH9, EQ5D_VAS)

placeholder <- rep(NA, 1093)
placeholder[which(CHAID_dataset$Improved_Symptoms == "yes")] <- "Improved"
placeholder[which(CHAID_dataset$Improved_Symptoms == "no")] <- "Not improved"
placeholder <- factor(placeholder,
                      levels = c("Not improved", "Improved"))
CHAID_dataset <- CHAID_dataset %>%
  mutate(Improvement = placeholder)

CHAID_dataset$Improved_Symptoms <- NULL

CHAID_dataset$DASH11 <- as.factor(CHAID_dataset$DASH11)
CHAID_dataset$DASH9 <- as.factor(CHAID_dataset$DASH9)

placeholder <- rep(NA, 1093)
CHAID_dataset$EQ5D_VAS <- round(CHAID_dataset$EQ5D_VAS)
placeholder[which(CHAID_dataset$EQ5D_VAS < 70)] <- "< 70"
placeholder[which(between(CHAID_dataset$EQ5D_VAS, 70, 79))] <- "70-79"
placeholder[which(between(CHAID_dataset$EQ5D_VAS, 80, 89))] <- "80-89"
placeholder[which(CHAID_dataset$EQ5D_VAS > 89)] <- "> 89"
placeholder <- factor(placeholder,
                      levels = c("< 70", "70-79", "80-89", "> 89"))
CHAID_dataset$EQ5D_VAS <- placeholder
